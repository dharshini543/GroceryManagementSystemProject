...
void padOrTrimString(char *dest, const char *src, size_t length)
{
    ...
    if (srcLen < length)
    {
        memset(dest +srcLen, ' ', length - srcLen); // Pad with spaces
    }

    dest[srcLen] = '\0';// Null-terminate
}
...
void loadInventoryFromFile(Inventory *inventory)
{
    ...
    while (fgets(buffer, RECORD_SIZE + 1, inventoryFile) != NULL)
    {
        ...
        {
            //printf("Skipping malformed record: %s\n", buffer);
            continue;
        }
        ...
        if (newItem->itemID < 0 || strlen(newItem->name) == 0 || strlen(newItem->brand) == 0)
        {
            printf("Skipping invalid or deleted item with ID %d.\n", newItem->itemID);
            free(newItem);
            continue;
        }
